...
export const UpdateUser = async (id, data) => {
    ...
    try {
        ...
        const url = `${process.env.REACT_APP_API_URL}/user/update-user/${id}`;
        console.log('API URL:', url); // Log URL API
        console.log('data:', data);
        const res = await axios.post(url, data, {
            headers: {
                token: `Bearer ${token}`, // Gửi token trong header
                'Content-Type': 'multipart/form-data', // Chú ý tới loại dữ liệu
            },
        });
